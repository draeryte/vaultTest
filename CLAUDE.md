...
**Routing.** `react-router-dom` drives navigation (`src/App.tsx` defines the tree; route components live in `src/app/routes/`, paths in `routes/paths.ts`). `RootLayout` runs the one-time session restore and holds rendering (the "Checking your session…" loader) until it settles, so guards decide against a stable auth state. `ProtectedRoute` (wraps `/dashboard`) redirects unauthenticated users to `/login`, stashing the attempted location so login round-trips back; `GuestRoute` (wraps `/login`) redirects already-authenticated users to `/dashboard`. Both guards just read `useIsAuthenticated()`. Logout and the global 401 handler clear the store, which makes `ProtectedRoute` redirect declaratively — no imperative navigation needed.
